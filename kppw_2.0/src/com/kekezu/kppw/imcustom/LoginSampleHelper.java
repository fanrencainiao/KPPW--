...
public class LoginSampleHelper
{
	...
	public static final String APP_KEY_TEST = B2BConstant.APPKEY.APPKEY_B2B; // 60026702
	...
	public void initSDK_Sample(Application context)
	{
		...
		// final String appkey = "taobao1234";
		...
		else if (type == TcmsEnvType.TEST)
		{
			...
			{
				YWAPI.aliInit(mApp, B2BConstant.APPKEY.APPKEY_B2B,
						AccountUtils.SITE_CNALICNH);
			}
		...
		YWSmilyMgr.setSmilyInitNotify(new YWSmilyMgr.SmilyInitNotify()
		{
			...
			public void onDefaultSmilyInitOk()
			{
				...
				SmilyCustomSample.setSmileySize(YWIMSmiley.SMILEY_TYPE_IMAGE,
						60);
	...
	private void sendAutoLoginState(YWLoginState loginState)
	{
		...
		// LocalBroadcastManager.getInstance(YWChannel.getApplication())
	...
	IYWContactCacheUpdateListener mContactCacheUpdateListener = new ContactCacheUpdateListenerImpl();

	/**
	 * 联系人相关操作通知回调，SDK使用方可以实现此接口来接收联系人操作通知的监听 所有方法都在UI线程调用
	...
	private void addContactListeners()
	{
		...
		if (mIMKit != null)
		{
			if (mContactOperateNotifyListener != null)
				mIMKit.getContactService().addContactOperateNotifyListener(
						mContactOperateNotifyListener);
			if (mContactCacheUpdateListener != null)
				mIMKit.getContactService().addContactCacheUpdateListener(
	...
	private void removeContactListeners()
	{
		...
		{
			if (mContactOperateNotifyListener != null)
				mIMKit.getContactService().removeContactOperateNotifyListener(
						mContactOperateNotifyListener);
			if (mContactCacheUpdateListener != null)
				mIMKit.getContactService().removeContactCacheUpdateListener(
						mContactCacheUpdateListener);
							...
							try
							{
								...
								if (object.has("text"))
								{
									...
									IMNotificationUtils.getInstance()
											.showToast(MyApp.getContext(),
													"透传消息，content = " + text);
								}
								else if (object.has("customizeMessageType"))
								{
									String customType = object
											.getString("customizeMessageType");
									if (!TextUtils.isEmpty(customType)
											&& customType
													.equals(ChattingOperationCustomSample.CustomMessageType.READ_STATUS))
									{
										YWConversation conversation = mIMKit
												.getConversationService()
												.getConversationByConversationId(
														message.getConversationId());
										long msgId = Long
												.parseLong(object
														.getString("PrivateImageRecvReadMessageId"));
										conversation.updateMessageReadStatus(
												conversation, msgId);
									}
								}
							}
