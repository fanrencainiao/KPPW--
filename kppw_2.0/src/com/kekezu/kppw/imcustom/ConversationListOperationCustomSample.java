...
import com.kekezu.kppw.activity.FragmentMsg;
import com.kekezu.kppw.activity.FragmentSystemList;
import com.kekezu.kppw.activity.SystemMsg;
...
public class ConversationListOperationCustomSample extends IMConversationListOperation
{
	...
	/**
	 * 返回自定义会话和群会话的默认头像 如返回本地的 R.drawable.test 该方法只适用设置自定义会话和群会话的头像，设置单聊会话头像请参考
	 * {@link com.taobao.openimui.sample.UserProfileSampleHelper}
	 * 
	 * @param fragment
	 * @param conversation
	...
	public int getConversationDefaultHead(Fragment fragment, YWConversation conversation)
	{
		...
		{
			YWCustomConversationBody body = (YWCustomConversationBody) conversation
					.getConversationBody();
			String conversationId = body.getIdentity();
			if (conversationId.equals(FragmentMsg.SYSTEM_TRIBE_CONVERSATION))
			{
				return R.drawable.aliwx_tribe_head_default;
			}
			else if (conversationId.equals(FragmentMsg.SYSTEM_FRIEND_REQ_CONVERSATION))
			{
	...
	public String getConversationName(Fragment fragment, YWConversation conversation)
	{
		if (conversation.getConversationBody() instanceof YWCustomConversationBody)
		{
			YWCustomConversationBody body = (YWCustomConversationBody) conversation
					.getConversationBody();
			if (body.getIdentity().equals(FragmentMsg.SYSTEM_TRIBE_CONVERSATION))
			{
				return "群系统消息";
			}
			else if (body.getIdentity()
					.equals(FragmentMsg.SYSTEM_FRIEND_REQ_CONVERSATION))
			{
				return "联系人系统消息";
			}
		}
	...
	public boolean onItemClick(Fragment fragment, YWConversation conversation)
	{
		...
		else if (type == YWConversationType.Custom)
		{
			if (conversation.getConversationType() == YWConversationType.Custom)
			{
				...
				if (body.getSubType() == 0)
				{
					Intent intent = new Intent(fragment.getActivity(), SystemMsg.class);
					intent.putExtra("type", "2");
					fragment.getActivity().startActivity(intent);
				}
				else if (body.getSubType() == 1)
				{
					Intent intent = new Intent(fragment.getActivity(), SystemMsg.class);
	 ...
	 */
	@Override
	public boolean onConversationItemLongClick(Fragment fragment,
