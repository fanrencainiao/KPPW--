 ...
 */
public class ContactOperateNotifyListenerImpl implements
		IYWContactOperateNotifyListener
{
	...
	{
		IMNotificationUtils.getInstance().showToast(MyApp.getContext(),
				contact.getUserId() + "用户请求加你为好友");

		// //增加未读数的显示
		// YWConversation conversation =
		// mIMKit.getConversationService().getCustomConversationByConversationId(SYSTEM_FRIEND_REQ_CONVERSATION);
		// if ( conversation!= null) {
		// YWCustomConversationUpdateModel model = new
		// YWCustomConversationUpdateModel();
		...
		// if(conversation.getConversationBody() instanceof
		// YWCustomConversationBody){
	...
	{
		IMNotificationUtils.getInstance().showToast(MyApp.getContext(),
				"云旺服务端（或其它终端）进行了好友添加操作对" + contact.getUserId());
