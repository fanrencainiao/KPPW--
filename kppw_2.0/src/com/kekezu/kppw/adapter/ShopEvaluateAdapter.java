...
public class ShopEvaluateAdapter extends BaseAdapter
{
	...
	public ShopEvaluateAdapter(Context context, ArrayList<HashMap<String, String>> list)
	{
	...
	public View getView(final int position, View convertView, ViewGroup parent)
	{
		...
		if (convertView == null)
		{
			view = LayoutInflater.from(context).inflate(R.layout.shop_detail_eval_item,
					parent, false);
		...
		holder.tv3.setText(list.get(position).get("desc").toString());
		holder.tv4.setText(list.get(position).get("created_at").toString());
		holder.ratingBar1.setRating(Float.valueOf(list.get(position).get("avg_score")
				.toString()));
		...
		Glide.with(context).load(list.get(position).get("avatar").toString())
				...
				.transform(new CircleTransform(context)).into(holder.imageView1);
